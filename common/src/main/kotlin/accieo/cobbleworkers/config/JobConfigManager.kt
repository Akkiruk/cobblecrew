...
/**
 * Loads and manages per-job config from JSON files under config/cobbleworkers/.
...
object JobConfigManager {
    ...
    private val configDir: Path = Paths.get("config", "cobbleworkers")
    ...
    fun load() {
        ...
        Cobbleworkers.LOGGER.info("[Cobbleworkers] Loaded ${configs.size} job configs from ${defaults.size} categories")
    }
    ...
    private fun loadCategoryFile(file: File, categoryDefaults: Map<String, JobConfig>) {
        ...
        try {
            ...
            Cobbleworkers.LOGGER.error("[Cobbleworkers] Failed to load config file ${file.name}: ${e.message}")
            configs.putAll(categoryDefaults)
        }
    }
